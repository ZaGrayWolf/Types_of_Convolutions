...
def calculate_flops_and_params(input_shape, output_channels, kernel_size, stride):
    ...
    # Parameters: (1x1 kernel size * in_channels + 1 bias) * output_channels
    params = (1 * 1 * in_channels + 1) * output_channels
    
    # Multiplications: (1 * 1 * in_channels) * out_height * out_width * output_channels
    mults = (1 * 1 * in_channels) * out_height * out_width * output_channels
    
    # Divisions: out_height * out_width * output_channels (for normalization)
    divs = out_height * out_width * output_channels
    
    # Additions and subtractions: (1 * 1 * in_channels - 1) * out_height * out_width * output_channels
    add_subs = (1 * 1 * in_channels - 1) * out_height * out_width * output_channels
...
def benchmark_convolution(input_shape, output_channels, kernel_size=(1, 1), stride=(1, 1)):
    model = nn.Conv2d(input_shape[2], output_channels, kernel_size, stride=stride)  # Pointwise Conv (1x1)
    model.eval()
